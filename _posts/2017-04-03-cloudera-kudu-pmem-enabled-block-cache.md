---
title: Kudu Persistent Memory Enabled Block Cache
author: sarahjelinek
layout: post
identifier: kudu_pmem_block_cache
---

# Using NVM Libraries To Add Persistent Memory Support to Cloudera Kudu Block Cache

An early project done with the NVM libraries was adding persistent memory support, both volatile and persistent mode, into the Cloudera Kudu storage engine block cache. This project required modification of existing third party code.
...
The volatile mode support for persistent memory has been fully integrated into the Kudu source base. The persistent mode support is not integrated but is ready, has been reviewed and is waiting for official integration from Cloudera.
...
Kudu is an open source storage engine, developed by Cloudera (www.cloudera.com), for structured data which supports low-latency random access together with efficient analytical access patterns.
...
struct KeyVal {
  uint64_t  key_len;
  uint64_t  value_len;
  // Really a boolean but for alignment reasons make it uint8_t.
...
```c
 if (IsPersistentMode()) {
    // At the time of insertion we know we have succeed in allocating
    ...
    if (!kv->valid) {
      ...
      // We persist the structure + key and value data  here once prior to 
      // setting the valid bit. At any point in time prior to this we can
      // fail and the valid bit will be set to 0 indicating that the entry
      // is not usable.
      pmemobj_persist(pop_, &kv, sizeof(struct KeyVal) + kv->value_len + kv->key_len);

      // Set the valid bit and persist. The valid bit indicates that the KeyVal structure
      // is complete and been persisted.
      kv->valid = 1;
      pmemobj_persist(pop_, &kv->valid, sizeof(uint8_t));
    }
  }
```
