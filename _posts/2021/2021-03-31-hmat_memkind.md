...
additional overhead from a interconnect, as in memory access between two sockets (`remote memory access`).
...
HMAT supports is required both in hardware and the OS*:
...
So far in memkind library, the existing memory kind determine the locality as the nearest NUMA based on the shortest NUMA
distance metrics.
...
Platforms may have multiple different types of memory, and each offering various bandwidth to the to the initiator of the memory request.
