...
additional overhead from a bus interconnection, as in memory access between two sockets (`remote memory access`).
...
However, most applications to mitigate bottlenecks are not interested in underlying hardware but rather in specific memory properties.
