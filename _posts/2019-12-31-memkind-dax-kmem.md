...
[libmemkind][memkind-release] extends its API by new kinds which allow for automatic detection and allocation
...
Using NVDIMM as an extension of DRAM needs to be configured with `ndctl` and `daxctl`.
...
After this operation, persistent memory will be configured as separate NUMA node and can be used as volatile memory.
For the example configuration below, persistent memory NUMA node is Node 3 (NUMA node to which does not belong any logical CPU)
...
persistent memory NUMA nodes are in the same shortest distance to the same CPU on which the process is allowed to run.
A check on that permission is done upon starting the application.
