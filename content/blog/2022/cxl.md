...
remote NUMA node access. At least for CXL attached DRAM. That's still plenty fast.
But is it fast enough for applications not to notice if suddenly some of its
memory accesses take twice (or more) as long? Only time will tell, but I'm not
so sure. For the most part, software that isn't NUMA-aware doesn't really scale
all that well across sockets.
...
Given that there might be many types of different memory in a system, there
...
Operating systems can parse that table and [expose performance information][linux-numa] [6]
to user-space software.

If you'd like to learn more, we've described HMAT in the context of memkind in
an [earlier article][memkind-blog] [7] on pmem.io.

## Implications for software

Armed with this knowledge, software should then be able to make informed data
placement decisions - which memory allocations go where.
...
But there to some caveats, in my opinion.
...
is [often advisable to increase performance][malloc-huge-page] [8]. You wouldn't
want your OS constantly moving 2MB objects between different memory tiers.
...
an optimized user-space software components to ensure hot and cold data is
...
There are now [many Linux][page-placement-linux] [9] kernel
[memory tiering][migrate-pages-linux] [10] activities. And just as many user-space
...
so that it can attach to existing regions instead of
just allocating new ones. It would also likely need some logic to make safe
updates to such regions. I hope that Persistent Memory Development Kit
...
[6] [Linux kernel guide - NUMA Locality][linux-numa]

[7] [Memkind support for heterogeneous memory attributes][memkind-blog]

[8] [Beyond malloc efficiency to fleet efficiency: a hugepage-aware memory allocator][malloc-huge-page]

[9] [Optimize Page Placement in Tiered Memory System][page-placement-linux]

[10] [Migrate Pages in lieu of discard][migrate-pages-linux]
