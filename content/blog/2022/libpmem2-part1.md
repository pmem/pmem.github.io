...
memory was becoming a reality. Initially, the libpmem
provided simple support for memory allocation operations on pmem. Over time,
development of hardware, software, and customer needs made it necessary to
evolve the library to include new features such as support for Windows,
[DevDax, RAS, eADR, and others][glossary-page]. The initial API design was no longer sufficient
and flexible. This led us to create an improved version of libpmem: **libpmem2**.
...
One of the big changes in *libpmem2* is the abandonment of this
mechanism in favor of a more intuitive and structured solution. The user now
...
reservations ([pmem2_config_set_vm_reservation][pmem2-config-set-vm-reservation]), which will be discussed in the in the next part
of this blog post series

## Meet granularity

Besides the mentioned configuration parameters there is the most important, mandatory one, called **granularity**.
It is set using function [pmem2_config_set_required_store_granularity][pmem2-config-set-required-store-granularity].
...
between the new and the old versions of the library. So far we have used the pmem/non-pmem distinction
...
[pmem2-get-drain-fn]: /pmdk/manpages/linux/master/libpmem2/pmem2_get_drin_fn.3
