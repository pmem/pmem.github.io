...
done with the **vmem_create**() function described below. The other
...
The **vmem_create**() function creates a memory pool and returns an opaque
...
**vmem_create**() can also be called with the **dir** argument pointing to a
...
there. As with **vmem_create**(), the minimum *size* allowed is defined
...
On success, **vmem_create**() returns an opaque memory pool handle of type
