...
The **vmem_check_versionU**()/**vmem_check_versionW**() function is used to see if the installed
...
When the version check is successful, **vmem_check_versionU**()/**vmem_check_versionW**() returns NULL.
Otherwise, **vmem_check_versionU**()/**vmem_check_versionW**() returns a static string describing the
...
from **vmem_errormsgU**()/**vmem_errormsgW**(). This function returns a pointer to a static buffer
